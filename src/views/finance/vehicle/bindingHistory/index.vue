  ...
  <div class="binding">
    ...
    <Modal v-model="modal" :title="modalTxt" @on-ok="ok">
      <Form ref="formCustom" :model="formCustom" :rules="ruleCustom" :label-width="80" v-if="reveal === 1 ? true : false">
        ...
        <FormItem label="退车日期" prop="vehicle_type">
          <DatePicker type="datetime" placeholder="请选择日期" style="width: 200px" @on-change="gainQuitDate" v-model="formCustom.withdrawDate" :editable="editable" clearable></DatePicker>
        </FormItem>
...
export default {
  ...
  methods: {
    inRowInfo(data) {
      ...
      this.formCustom.withdrawDate = data.withdrawDate
          ...
          if (res.results[j].active_date !== undefined && res.results[j].withdraw_date !== undefined) {
            ...
            if (res.results.length - 1 > 0) {
              if (j !== 0) {
                ...
                num += Number(res.results[j].skyF)
              } else {
                num += 0
                // eslint-disable-next-line
                let query_car = Bmob.Query('car')
                query_car.set('id', JSON.parse(sessionStorage.getItem('rowInfo')).objectId)
                query_car.set('idle_cost', num.toString())
                query_car.save()
                this.pageUser(1)
                res.results[j].sky = 0
    ...
    // 选择时间触发
    gainQuitDate(v) {
      if (this.active > this.withdraw) {
        this.withdraw = ''
        this.$Message.warning('退车日期不能小于提车日期')
      } else {
        this.formCustom.withdrawDate = v
      }
    },
    // 修改
    alter() {
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          query.set('withdraw_date', {
            '__type': 'Date',
            'iso': this.formCustom.withdrawDate
          })
        ...
        if (this.formCustomCat.realNamemodel !== this.carHistoryInfo[0].realName) {
          ...
          query_car.relation('car').then(res => {
            ...
            for (let k in res.results[0]) {
              if (k !== 'objectId' && k !== 'updatedAt' && k !== 'createdAt' && k !== 'bind_user' && k !== 'realName') {
