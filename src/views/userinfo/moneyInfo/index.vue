<template>
  <div class="moneyinfo">
    ...
    <Table border :columns="columns" :data="moneydata" @on-row-click="inRowInfo"></Table>
    ...
    <Modal v-model="modal" :title="remText" @on-ok="ok">
      <Form ref="formCustom" :model="formCustom" :rules="ruleCustom" :label-width="80" v-if="eliminateNum === 2? true : false">
      ...
      <Form ref="formCustomLiu" :model="formCustomLiu" :rules="ruleCustomLiu" :label-width="80" v-if="eliminateNum === 1? true : false">
        <FormItem label="流水" prop="liushui">
          <Input type="text" v-model="formCustomLiu.liushui" placeholder="请输入数量"></Input>
        </FormItem>
        <FormItem label="消费" prop="xiaofei">
          <Input type="text" v-model="formCustomLiu.xiaofei" placeholder="请输入数量"></Input>
        </FormItem>
        <FormItem label="工作状态" prop="work">
          <Select v-model="formCustomLiu.work" style="width:200px">
            <Option value="1">出勤</Option>
            <Option value="2">休息</Option>
          </Select>
        </FormItem>
        <FormItem label="扣罚" prop="koufa">
          <Input type="text" v-model="formCustomLiu.koufa" placeholder="请输入数量"></Input>
        </FormItem>
        <FormItem label="备注" prop="beizhu">
          <Input type="text" v-model="formCustomLiu.beizhu" placeholder="请输入扣罚原因"></Input>
        </FormItem>
      </Form>
    </Modal>
  </div>
...
export default {
  ...
  methods: {
    ...
    inRowInfo(data) {
      sessionStorage.setItem('dataLui', JSON.stringify(data))
      if (this.trim === 1) {
        this.formCustomLiu.liushui = data.liushui.toString()
        this.formCustomLiu.xiaofei = data.xiaofei.toString()
        if (data.koufa !== undefined) {
          this.formCustomLiu.koufa = data.koufa.toString()
        } else {
          this.formCustomLiu.koufa = '0'
        }
        this.formCustomLiu.beizhu = data.beizhu
        if (data.on_duty === '出勤') {
          this.formCustomLiu.work = '1'
        } else if (data.on_duty === '休息') {
          this.formCustomLiu.work = '2'
        }
      }
    },
    select(v) {
    ...
    search() {
      ...
      let query = Bmob.Query('commission')
      if (this.sun / 1000 > 1) {
        for (let i = 0, max = Math.floor(this.sun / 1000); i < max; i++) {
          query.skip(i * 1000)
          query.statTo('limit', `1000`)
        }
      } else {
        query.statTo('limit', `1000`)
      }
    ...
    gatDate(v) {
      ...
      for (let i = 0, max = this.moneydata.length; i < max; i++) {
        if (v.replace(/\-/g, '/') === this.moneydata[i].date) {
          this.judge = 1
        }
      }
    },
    ok() {
      if (this.eliminateNum === 1 && this.trim === 1) {
        this.handleSubmitLui('formCustomLiu')
      } else {
        if (this.judge !== 1) {
    ...
    handleSubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          query.set('upload_date', {
            '__type': 'Date',
            'iso': `${this.formCustom.date} 00:00:00`
          })
    ...
    // 流水修改
    handleSubmitLui(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          let _this = this
          let objectId = JSON.parse(sessionStorage.getItem('dataLui')).objectId
          // eslint-disable-next-line
          let query = Bmob.Query('commission')
          query.set('id', objectId)
          query.set('liushui', Number(this.formCustomLiu.liushui))
          query.set('xiaofei', Number(this.formCustomLiu.xiaofei))
          query.set('koufa', Number(this.formCustomLiu.koufa))
          query.set('beizhu', this.formCustomLiu.beizhu)
          let totalPay = Number(this.formCustomLiu.liushui) - (Number(this.formCustomLiu.xiaofei) + Number(this.formCustomLiu.koufa))
          query.set('totalPay', totalPay)
          query.set('on_duty', this.formCustomLiu.work)
          // eslint-disable-next-line
          query.save().then(res => {
            this.$Modal.warning({
              title: '修改成功',
              onOk() {
                _this.pageUserInfo(JSON.parse(sessionStorage.getItem('currentPage')))
